...
namespace Project.Runtime
{
    public class Demo : MonoBehaviour
    {
        ...
        private ComponentPool<HealthBar, HealthBarPrefab> _healthBarPool;
        ...
        async void Start()
        {
            ...
            _healthBarPool = SharedPool.Of<ComponentPool<HealthBar, HealthBarPrefab>>();
            this._healthBarPool.Prefab = new HealthBarPrefab() {Parent = this.canvas, PrepoolAmount = 10};
            for (int i = 0; i < 10; i++)
            {
                var h = await _healthBarPool.Rent();
                h.SetHealth(i * 100);
            }
        }
        ...
        private async UniTask UpdateRentNewItems()
        {
            ...
            var newHealthBarPool = SharedPool.Of<ComponentPool<HealthBar, HealthBarPrefab>>();
            var healthBar = await newHealthBarPool.Rent();
            healthBar.SetHealth(999);
            this._healthBarPool.Return(canvas.GetChild(0).GetComponent<HealthBar>());
            this._healthBarPool.Return(canvas.GetChild(3).GetComponent<HealthBar>());
            this._healthBarPool.Return(canvas.GetChild(4).GetComponent<HealthBar>());
        ...
        private void OnGUI()
        {
            if (GUILayout.Button("Update Stack Pool 1"))
            {
                ...
                Debug.Log(stackPool.GetHashCode());
